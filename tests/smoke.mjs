...
const offensiveUrl = new URL("offensive-bfm.html", targetUrl).href;
const debuggerBase = process.env.CDP_URL ?? "http://127.0.0.1:9222";
const projectRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const EXPECTED_MISSION_COUNT = 18;
const EXPECTED_FIGURE_COUNT = 3;
const EXPECTED_OFFENSIVE_MISSION_COUNT = 11;
const EXPECTED_OFFENSIVE_FIGURE_COUNT = 4;
...
async function run() {
  ...
  const offensiveMissionSourceDirectory = path.join(projectRoot, "src", "offensive-missions");
  const missionSources = fs.readdirSync(missionSourceDirectory).filter((name) => /^\d{2}-.+\.html$/.test(name));
  const offensiveMissionSources = fs.readdirSync(offensiveMissionSourceDirectory).filter((name) => /^\d{2}-.+\.html$/.test(name));
  const appTemplateSource = fs.readFileSync(path.join(projectRoot, "src", "app.template.html"), "utf8");
  const offensiveAppTemplateSource = fs.readFileSync(path.join(projectRoot, "src", "offensive-app.template.html"), "utf8");
  const publicIndexSource = fs.readFileSync(path.join(projectRoot, "index.html"), "utf8");
  const publicOffensiveSource = fs.readFileSync(path.join(projectRoot, "offensive-bfm.html"), "utf8");
  const generatedContentSource = fs.readFileSync(path.join(projectRoot, "scripts", "generated-content.js"), "utf8");
  const generatedOffensiveContentSource = fs.readFileSync(path.join(projectRoot, "scripts", "generated-offensive-content.js"), "utf8");
  ...
  const publicOffensiveLines = publicOffensiveSource.trimEnd().split(/\r?\n/).length;
  record(
    "Part 1とPart 2を独立した章ソースから生成",
    missionSources.length === EXPECTED_MISSION_COUNT &&
      (appTemplateSource.match(/@include missions\//g) || []).length === EXPECTED_MISSION_COUNT &&
      offensiveMissionSources.length === EXPECTED_OFFENSIVE_MISSION_COUNT &&
      (offensiveAppTemplateSource.match(/@include offensive-missions\//g) || []).length === EXPECTED_OFFENSIVE_MISSION_COUNT &&
      publicIndexSource.includes("Generated by tools/build-html.mjs") &&
      publicOffensiveSource.includes("Generated by tools/build-html.mjs") &&
      generatedContentSource.includes("Generated by tools/build-html.mjs") &&
      generatedOffensiveContentSource.includes("Generated by tools/build-html.mjs") &&
      generatedContentSource.includes("window.BFM.generatedContent") &&
      generatedOffensiveContentSource.includes("window.BFM.generatedContent") &&
      !publicIndexSource.includes("@include missions/") &&
      !publicOffensiveSource.includes("@include offensive-missions/"),
    JSON.stringify({
      foundationMissionSources: missionSources.length,
      offensiveMissionSources: offensiveMissionSources.length,
  ...
  record(
    "offensive-bfm.htmlを独立した表示シェルへ短縮",
    publicOffensiveLines <= 100 && Buffer.byteLength(publicOffensiveSource) < 10000 &&
      publicOffensiveSource.includes("data-generated-content-root") &&
      publicOffensiveSource.includes('scripts/generated-offensive-content.js') &&
      publicOffensiveSource.includes('scripts/content-loader.js') &&
      !publicOffensiveSource.includes('class="module"'),
    JSON.stringify({ lines: publicOffensiveLines, bytes: Buffer.byteLength(publicOffensiveSource) })
  );
  record(
    ...
    robotsSource.includes("Sitemap: https://bfm-starters.netlify.app/sitemap.xml") &&
      ...
      sitemapSource.includes("<loc>https://bfm-starters.netlify.app/offensive-bfm.html</loc>") &&
  ...
  const basics = await evaluate(socket, `(() => ({
    ...
    siteGuide: (() => {
      ...
      return {
        ...
        offensiveRoute: [...(guide?.querySelectorAll(".part-card a") || [])]
          .find((link) => link.textContent.includes("Part 2"))?.href || ""
      };
    })(),
    punctuationPolicy: {
      ...
      return {
        ...
        validTargets: links.every((link) => {
          ...
          if (url.pathname === location.pathname) {
            return url.hash.length > 1 && Boolean(document.getElementById(url.hash.slice(1)));
          }
          return url.pathname.endsWith("/offensive-bfm.html") &&
            (!url.hash || url.hash === "#offensive-overview");
        }),
        offensiveRoute: links.find((link) => link.textContent.includes("Offensive BFM"))?.href || ""
      };
    })(),
  ...
  record(
    ...
    basics.seo.description.includes("BFM Japan") &&
      ...
      basics.seo.description.includes("Offensive BFM") &&
  ...
  record("Part 1の確認問題が12問", basics.quiz === 12, basics.quiz);
  ...
  record(
    "Part 1の18章にスクロール導入画面を表示",
    basics.missionReveals.sequences === EXPECTED_MISSION_COUNT && basics.missionReveals.reveals === EXPECTED_MISSION_COUNT &&
      basics.missionReveals.modules === EXPECTED_MISSION_COUNT && basics.missionReveals.orderValid &&
      ...
      basics.missionReveals.labels.every((label, index) =>
        label.endsWith(`MISSION ${String(index + 1).padStart(2, "0")}`)
  ...
  record(
    ...
    basics.componentArchitecture.componentStylesheet && basics.componentArchitecture.layeredStylesheets &&
      basics.componentArchitecture.lessonHeaders === EXPECTED_MISSION_COUNT &&
      ...
      basics.componentArchitecture.cardGrids >= 12 && basics.componentArchitecture.quizCards === 12 &&
      basics.componentArchitecture.glossaryCards >= 64 && basics.componentArchitecture.courseCards === 2 &&
  ...
  record(
    "全章を番号と題名の統合見出しに統一",
    basics.numberedModuleHeadings.length === EXPECTED_MISSION_COUNT &&
  ...
  record(
    ...
    basics.siteGuide.firstMission === "site-guide" &&
      ...
      Boolean(basics.siteGuide.offensiveRoute) &&
      new URL(basics.siteGuide.offensiveRoute).pathname.endsWith("/offensive-bfm.html"),
    JSON.stringify(basics.siteGuide)
  );
  record(
    "左目次とスプリッターを撤去し右上を目次に変更",
    basics.headerToc.menu && basics.headerToc.panel && basics.headerToc.links === EXPECTED_MISSION_COUNT &&
  ...
  record(
    ...
    basics.missionIndexTypography.length === EXPECTED_MISSION_COUNT && basics.missionIndexMatches && basics.missionIndexTypography.every((item) =>
  ...
  record(
    ...
    introCoverage.selectorsPresent && introCoverage.conceptsPresent &&
      introCoverage.glossaryCount >= 64 && introCoverage.missionCount === EXPECTED_MISSION_COUNT,
  ...
  const references = await evaluate(socket, `(async () => {
    ...
    return {
      ...
      markersValid: lessonMarkers.length >= 6 && lessonMarkers.every((marker) => {
  ...
  record("Part 1の出典付き公開画像が3点", references.figureCount === EXPECTED_FIGURE_COUNT, references.figureCount);
  record(
    "Part 1の3画像は小さな参照番号で第17章へリンク",
  ...
  record(
    "参考情報と外部リンクは第17章へ集約",
    references.referenceItems === 10 && references.referenceLinkCount >= 14 &&
  ...
  const courseFlow = await evaluate(socket, `(() => {
    ...
    const offensiveLink = document.querySelector("#course-offensive");
    return {
      ...
      offensiveRoute: offensiveLink?.href || "",
      offensiveLive: Boolean(offensiveLink && !offensiveLink.hasAttribute("aria-disabled"))
    };
  })()`);
  record(
    "基礎修了後に2コースを選択できる",
    courseFlow.afterQuiz && courseFlow.hasOffensive && courseFlow.hasDefensive && courseFlow.resultLinkText.length > 0 &&
      courseFlow.offensiveLive && Boolean(courseFlow.offensiveRoute) &&
      new URL(courseFlow.offensiveRoute).pathname.endsWith("/offensive-bfm.html"),
  ...
  const offensiveLoaded = new Promise((resolve) => {
    const listener = (event) => {
      const message = JSON.parse(event.data);
      if (message.method === "Page.loadEventFired") {
        socket.removeEventListener("message", listener);
  ...
  await command(socket, "Page.navigate", { url: offensiveUrl });
  await Promise.race([offensiveLoaded, wait(8000)]);
  await wait(500);
  await setViewport(socket, 1440, 1000);
  await evaluate(socket, "window.scrollTo(0, 0)");
  await wait(100);

  const offensive = await evaluate(socket, `(async () => {
    const ids = [
      "offensive-overview", "offensive-geometry", "offensive-entry", "offensive-yoyos",
      "offensive-overshoot", "offensive-responses", "high-aspect", "offensive-recap",
      "offensive-quiz", "offensive-references", "defensive-next"
    ];
    const sections = ids.map((id) => document.getElementById(id));
    const indexLinks = [...document.querySelectorAll(".mission-index-list a")];
    const headings = [...document.querySelectorAll(".module-header .numbered-module-title")];
    const figures = [...document.querySelectorAll(".reference-figure")];
    ...
    const referenceSection = document.querySelector("#offensive-references");
    const referenceItems = [...(referenceSection?.querySelectorAll(".references-list > li") || [])];
    const lessonMarkers = [...document.querySelectorAll(
      '.mission-sequence:not(#offensive-references) .source-marker'
    )];
    const firstMarkerOrder = lessonMarkers.reduce((order, marker) => {
      const value = Number(marker.textContent.trim().slice(1, -1));
      if (Number.isInteger(value) && !order.includes(value)) order.push(value);
      return order;
    }, []);
    ...
    const foundationLink = navLinks.find((link) => link.textContent.includes("BFM入門"));
    const offensiveLink = navLinks.find((link) => link.textContent.includes("Offensive BFM"));
    const localHashLinks = [...document.querySelectorAll('a[href^="#"]')];
    const expectedStylesheetOrder = [
      "styles/tokens.css", "styles/base.css", "styles/layout.css", "styles/components.css",
      "styles/welcome.css", "styles/lessons.css", "styles/quiz.css", "styles/glossary.css",
      "styles/responsive.css"
    ];
    const stylesheetOrder = [...document.querySelectorAll('link[rel="stylesheet"]')]
      .map((link) => new URL(link.href).pathname.split("/").slice(-2).join("/"));
    const scripts = [...document.scripts]
      .map((script) => script.src && new URL(script.src).pathname)
      .filter((pathname) => pathname && pathname.includes("/scripts/") && pathname.endsWith(".js"));
    const isHorizontalComparison = (selector, expectedCount) => {
      const items = [...document.querySelectorAll(selector)];
      const rects = items.map((item) => item.getBoundingClientRect());
      return items.length === expectedCount &&
        Math.max(...rects.map((rect) => rect.top)) - Math.min(...rects.map((rect) => rect.top)) <= 2 &&
        rects.every((rect, index) => index === 0 || rect.left > rects[index - 1].left);
    };
    const logicalFlows = [...document.querySelectorAll(".fundamentals-path.logic-flow")].map((flow) => {
      const rects = [...flow.children].map((item) => item.getBoundingClientRect());
      return {
        steps: rects.length,
        vertical: rects.length >= 3 && rects.every((rect, index) => index === 0 ||
    ...
    return {
      ...
      orderValid: sections.every((section, index) => index === 0 ||
        Boolean(sections[index - 1].compareDocumentPosition(section) & Node.DOCUMENT_POSITION_FOLLOWING)),
      missionCount: document.querySelectorAll(".mission-sequence[data-section]").length,
      revealCount: document.querySelectorAll("[data-mission-reveal]").length,
      moduleCount: document.querySelectorAll("[data-mission-content]").length,
      numberContracts: sections.every((section, index) => {
        const expected = String(index + 1).padStart(2, "0");
        const toc = indexLinks[index];
        const reveal = section?.querySelector(".mission-reveal-label span")?.textContent.trim();
        const heading = section?.querySelector(".module-title-number")?.textContent.trim();
        const tocTarget = toc?.getAttribute("href")?.slice(1);
        const tocNumber = toc?.querySelector("span")?.textContent.trim();
        const tocClone = toc?.cloneNode(true);
        tocClone?.querySelector("span")?.remove();
        const title = section?.querySelector(".module-title-text")?.textContent.trim();
        return reveal === expected && heading === expected + ":" && tocNumber === expected &&
          tocTarget === section?.id && tocClone?.textContent.trim() === title;
      }),
      titlesMatch: sections.every((section) =>
        section?.querySelector(".mission-reveal-title")?.textContent.trim() ===
        section?.querySelector(".module-title-text")?.textContent.trim()
      ),
      headings: headings.length,
      headingTypography: headings.every((heading) => {
        const number = heading.querySelector(".module-title-number");
        const title = heading.querySelector(".module-title-text");
        return number && title && parseFloat(getComputedStyle(number).fontSize) > parseFloat(getComputedStyle(title).fontSize) &&
          number.getBoundingClientRect().bottom <= title.getBoundingClientRect().top + 1 &&
          title.getClientRects().length === 1;
      }),
      tocLinks: indexLinks.length,
      tocSummary: document.querySelector("#mission-index-menu > summary")?.textContent.trim() || "",
      generatedContentMounted: document.documentElement.dataset.contentMounted === "true" &&
        window.BFM.contentMounted === true && !document.querySelector("[data-generated-content-root]"),
      stylesheetOrderValid: expectedStylesheetOrder.every((name, index) => stylesheetOrder[index] === name),
      scriptCount: scripts.length,
      firstScript: scripts[0] || "",
      quizCards: document.querySelectorAll("#offensive-quiz-list .quiz-card").length,
      conceptsPresent: [
        "Exclusive-use Turning Room", "Attack Window", "High Yo-Yo", "Low Yo-Yo",
        "Lag Displacement Roll", "Flat Scissors", "Rolling Scissors", "Redefinition",
        "High-Aspect BFM"
      ].every((term) => text.toLowerCase().includes(term.toLowerCase())),
      supplementalTermsPresent: [
        "LOSR", "Reversal opportunity", "Point of Departure", "Role reversal",
        "Neutralized", "Lead Turn", "Downrange travel"
      ].every((term) => text.toLowerCase().includes(term.toLowerCase())),
      termDefinitionsPresent: Boolean(
        document.querySelector("#offensive-entry .term-support-grid") &&
      ...
      excludedTerms: ["3/9", "fog of war", "wvr", "padlock", "lookout", "索敵"]
        .filter((term) => document.body.innerText.toLowerCase().includes(term)),
      figures: figures.map((figure) => {
        const image = figure.querySelector("img");
        const marker = figure.querySelector('figcaption .source-marker[href^="#"]');
        return {
          loaded: Boolean(image?.complete && image.naturalWidth > 0),
          alt: image?.alt.trim() || "",
          marker: marker?.getAttribute("href") || "",
          markerTargetPresent: Boolean(marker && document.querySelector(marker.getAttribute("href")))
        };
      }),
      referenceItems: referenceItems.length,
      referenceIds: referenceItems.map((item) => item.id),
      referenceIdsUnique: new Set(referenceItems.map((item) => item.id)).size === referenceItems.length,
      externalReferencesValid: [...(referenceSection?.querySelectorAll('a[href^="https://"]') || [])].every((link) =>
        link.target === "_blank" && link.relList.contains("noopener") && link.relList.contains("noreferrer")
      ),
      firstMarkerOrder,
      markersValid: lessonMarkers.length >= 4 && lessonMarkers.every((marker) => {
        const href = marker.getAttribute("href") || "";
        const target = href.startsWith("#") ? document.querySelector(href) : null;
        const label = marker.textContent.trim();
        return Boolean(target && label.startsWith("[") && label.endsWith("]") &&
          Number.isInteger(Number(label.slice(1, -1))));
      }),
      localLinksValid: localHashLinks.every((link) => {
        const href = link.getAttribute("href");
        return href === "#" || Boolean(document.querySelector(href));
      }),
      foundationRoute: foundationLink?.href || "",
      offensiveRouteLocal: offensiveLink?.getAttribute("href") === "#offensive-overview",
  ...
  record(
    "Offensive BFMを独立したPart 2ページとして表示",
    offensive.title.includes("Offensive BFM") && offensive.title.includes("BFM Japan") &&
      offensive.lang === "ja" && offensive.h1Count === 1 && offensive.h1Text.includes("Offensive BFM") &&
      offensive.heroBeforeMissions && offensive.foundationContentAbsent,
  ...
  record(
    ...
    offensive.seo.description.includes("Offensive BFM") &&
      ...
      offensive.seo.ogUrl === "https://bfm-starters.netlify.app/offensive-bfm.html" &&
      offensive.seo.structuredData?.name.includes("Offensive BFM") &&
      offensive.seo.structuredData?.url === "https://bfm-starters.netlify.app/offensive-bfm.html" &&
      offensive.seo.structuredData?.inLanguage === "ja",
    JSON.stringify(offensive.seo)
  );
  record(
    "Part 2はMission 01〜11を独立して構成",
    offensive.idsPresent && offensive.orderValid &&
      offensive.missionCount === EXPECTED_OFFENSIVE_MISSION_COUNT &&
      offensive.revealCount === EXPECTED_OFFENSIVE_MISSION_COUNT &&
      offensive.moduleCount === EXPECTED_OFFENSIVE_MISSION_COUNT &&
      offensive.headings === EXPECTED_OFFENSIVE_MISSION_COUNT &&
      offensive.tocLinks === EXPECTED_OFFENSIVE_MISSION_COUNT &&
      offensive.numberContracts && offensive.titlesMatch && offensive.headingTypography &&
      offensive.tocSummary === "目次",
    JSON.stringify(offensive)
  );
  record(
    "Part 2も共通コンポーネントと専用生成データを使用",
    offensive.generatedContentMounted && offensive.stylesheetOrderValid && offensive.scriptCount === 14 &&
      offensive.firstScript.endsWith("/scripts/generated-offensive-content.js") && offensive.quizCards === 8,
    JSON.stringify(offensive)
  );
  record(
    "Offensive BFMを順序立てて説明し操作数値を一般化",
    offensive.conceptsPresent && offensive.operationalValuesAbsent && offensive.excludedTerms.length === 0,
    JSON.stringify({
      conceptsPresent: offensive.conceptsPresent,
      operationalValuesAbsent: offensive.operationalValuesAbsent,
      excludedTerms: offensive.excludedTerms
    })
  );
  record(
    "Part 2は比較を横、判断手順を縦に配置",
    offensive.layoutContracts.entryTimingHorizontal &&
  ...
  record(
    "Part 2の初出専門語を本文内で補足",
    offensive.supplementalTermsPresent && offensive.termDefinitionsPresent,
    JSON.stringify({
  ...
  record(
    "Part 2の出典付き公開画像が4点",
    offensive.figures.length === EXPECTED_OFFENSIVE_FIGURE_COUNT && offensive.figures.every((figure) =>
      figure.loaded && figure.alt.length >= 12 && /^#offensive-ref-[1-4]$/.test(figure.marker) && figure.markerTargetPresent
    ),
    JSON.stringify(offensive.figures)
  );
  record(
    "Part 2の参考資料4件と参照番号をページ内に集約",
    offensive.referenceItems === 4 && offensive.referenceIdsUnique &&
      offensive.externalReferencesValid && offensive.markersValid &&
      offensive.firstMarkerOrder.join("|") === "1|2|3|4" &&
      offensive.referenceIds.join("|") ===
        "offensive-ref-1|offensive-ref-2|offensive-ref-3|offensive-ref-4",
    JSON.stringify({
      referenceItems: offensive.referenceItems,
      referenceIds: offensive.referenceIds,
      firstMarkerOrder: offensive.firstMarkerOrder
    })
  );
  record(
    "Part 2の目次・ページ内リンク・Part 1への戻り先が有効",
    offensive.localLinksValid && offensive.offensiveRouteLocal && Boolean(offensive.foundationRoute) &&
      new URL(offensive.foundationRoute).pathname.endsWith("/index.html") &&
      new URL(offensive.foundationRoute).hash === "#overview" &&
      offensive.navLabels.join("|") === "BFM入門|Offensive BFM|Defensive BFM" &&
      offensive.duplicateIds.length === 0 && offensive.brokenAriaRefs.length === 0 &&
      offensive.imagesMissingAlt === 0 && offensive.unlabeledButtons === 0,
    JSON.stringify(offensive)
  );

  const offensiveCorrectAnswers = [1, 1, 0, 1, 1, 1, 1, 1];
  for (let index = 0; index < offensiveCorrectAnswers.length; index += 1) {
    await evaluate(socket, `(() => {
      const list = document.querySelector("#offensive-quiz-list");
      const card = list.querySelector('[data-quiz-index="${index}"]');
      card.querySelectorAll('input[type="radio"]')[${offensiveCorrectAnswers[index]}].click();
      card.querySelector(".quiz-submit").click();
    })()`);
  }
  const offensiveQuizResult = await evaluate(socket, `(() => ({
    score: document.querySelector("#offensive-quiz-score").textContent,
    hidden: document.querySelector("#offensive-quiz-result").hidden,
    answered: document.querySelector("#offensive-quiz-answered").textContent,
    nextLink: document.querySelector('#offensive-quiz-message a[href="#defensive-next"]')?.textContent.trim() || ""
  }))()`);
  record(
    "Part 2の8問を独立して採点",
    offensiveQuizResult.score === "8" && !offensiveQuizResult.hidden &&
      offensiveQuizResult.answered.startsWith("8") && offensiveQuizResult.nextLink.length > 0,
    JSON.stringify(offensiveQuizResult)
  );

  if (process.env.SCREENSHOT_DIR) {
    await evaluate(socket, 'document.documentElement.style.scrollBehavior = "auto"');
    for (const [id, filename] of [
      ["part-two-hero", "bfm-offensive-desktop-hero.png"],
      ["offensive-yoyos", "bfm-offensive-desktop-yoyos.png"],
      ["offensive-references", "bfm-offensive-desktop-references.png"]
    ]) {
      await evaluate(socket, `(() => {
        const section = document.querySelector("#${id}");
        const target = section.querySelector("[data-mission-content]") || section;
  ...
  await evaluate(socket, "window.scrollTo(0, 0)");
  await wait(100);
  const offensiveMobile = await evaluate(socket, `(() => {
    const menuButton = document.querySelector("#menu-toggle");
    menuButton.click();
    const menuOpened = menuButton.getAttribute("aria-expanded") === "true" &&
      document.querySelector("#global-nav").classList.contains("is-open");
    document.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true }));
    const withinViewport = (element) => {
      const rect = element.getBoundingClientRect();
      return rect.left >= -1 && rect.right <= innerWidth + 1;
    ...
    return {
      menuOpened,
      menuClosed: menuButton.getAttribute("aria-expanded") === "false",
      overflow: document.documentElement.scrollWidth - document.documentElement.clientWidth,
      h1Visible: document.querySelector("h1").getBoundingClientRect().width > 0,
      tocVisible: getComputedStyle(toc).display !== "none" && withinViewport(toc),
      touchTargets: [menuButton, toc, ...document.querySelectorAll(".quiz-option, .quiz-submit")]
        .filter((element) => getComputedStyle(element).display !== "none")
        .every((element) => {
          const rect = element.getBoundingClientRect();
          return rect.width >= 44 && rect.height >= 44 && withinViewport(element);
        }),
      revealsFit: [...document.querySelectorAll(".mission-reveal-title")].every((element) =>
        element.scrollWidth <= element.clientWidth + 1 && withinViewport(element)
      ),
      lessonTitlesFit: [...document.querySelectorAll(".lesson-title-text")].every((element) =>
        element.scrollWidth <= element.clientWidth + 1 && withinViewport(element)
      ),
      tablesFit: [...document.querySelectorAll(".table-wrap")].every(withinViewport),
      wordsProtected: [...document.querySelectorAll(".lesson-title-text, .term-en, .course-choice h3")].every((element) => {
        const style = getComputedStyle(element);
        return style.wordBreak !== "break-all" && style.overflowWrap !== "anywhere" && style.hyphens !== "auto";
      })
    };
  })()`);
  record(
    "Part 2は375pxでもナビゲーションと本文が崩れない",
    offensiveMobile.menuOpened && offensiveMobile.menuClosed && offensiveMobile.overflow <= 1 &&
      offensiveMobile.h1Visible && offensiveMobile.tocVisible && offensiveMobile.touchTargets &&
      offensiveMobile.revealsFit && offensiveMobile.lessonTitlesFit && offensiveMobile.tablesFit &&
      offensiveMobile.wordsProtected,
    JSON.stringify(offensiveMobile)
  );

  await setViewport(socket, 320, 568);
  const offensiveNarrow = await evaluate(socket, `(() => ({
    overflow: document.documentElement.scrollWidth - document.documentElement.clientWidth,
    h1Visible: document.querySelector("h1").getBoundingClientRect().width > 0
  }))()`);
  record(
    "Part 2は320pxでも横オーバーフローなし",
    offensiveNarrow.overflow <= 1 && offensiveNarrow.h1Visible,
    JSON.stringify(offensiveNarrow)
  );

  await evaluate(socket, "window.scrollTo(0, document.documentElement.scrollHeight)");
  await wait(120);
  const offensiveProgress = await evaluate(socket, `(() => ({
    value: Number(document.querySelector(".index-progress").getAttribute("aria-valuenow")),
    visualValue: Number(document.querySelector(".reading-progress").getAttribute("aria-valuenow")),
    text: document.querySelector("#progress-percent").textContent
  }))()`);
  record(
    "Part 2のページ進捗が100%付近",
    offensiveProgress.value >= 99 && offensiveProgress.visualValue >= 99 && offensiveProgress.text.includes("100"),
    JSON.stringify(offensiveProgress)
  );
  ...
  const noScriptStates = [];
  for (const [page, url] of [["Part 1", targetUrl], ["Part 2", offensiveUrl]]) {
    ...
    noScriptStates.push({ page, ...(await evaluate(socket, `(() => ({
  ...
  record(
    "両ページでJavaScript無効時は有効化案内を表示",
    noScriptStates.length === 2 && noScriptStates.every((state) =>
      !state.initialized && state.moduleCount === 0 && state.mountPresent && state.noticeVisible &&
      state.noticeText.includes("JavaScriptを有効にしてください") && state.noticeText.includes("再読み込みしてください")
    ),
    JSON.stringify(noScriptStates)
  );
